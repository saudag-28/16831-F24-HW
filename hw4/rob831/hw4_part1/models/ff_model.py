...
class FFModel(nn.Module, BaseModel):
    ...
    def get_prediction(self, obs, acs, data_statistics):
        ...
        prediction, _ = self.forward(obs, acs, data_statistics['obs_mean'],
                                    ...
                                    data_statistics['delta_std'])# TODO(Q1) get the predicted next-states (s_t+1) as a numpy array
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        ...
        _, delta_pred_norm = self.forward(observations, actions, data_statistics['obs_mean'],
                                    data_statistics['obs_std'], data_statistics['acs_mean'],
                                    data_statistics['acs_std'], data_statistics['delta_mean'],
                                    data_statistics['delta_std'])

        target = ptu.from_numpy(target)
        loss = self.loss(delta_pred_norm, target)# TODO(Q1) compute the loss
        # Hint: `self(...)` returns a tuple, but you only need to use one of the
        # outputs.
